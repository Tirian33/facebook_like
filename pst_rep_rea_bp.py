...
def make_post():
    ...
    target_account = Account.query.filter_by(id = request.form.get('postedOnID'), deleted_at = None).first()
...
def edit_post(post_id):
    ...
    target_post = Post.query.filter_by(id=post_id, deleted_at = None, poster_id = get_jwt_identity()).first()
...
def delete_post(post_id):
    ...
    if (target_post.posted_on_id == get_jwt_identity() or target_post.poster_id == get_jwt_identity()):
        #DEL
...
def make_reply():
    ...
    target_post = Post.query.filter_by(id = request.form.get('respTo'), deleted_at = None).first()
    if target_post is None:
        abort(400, "The post you are trying to respond to has been deleted.")

    target_account = Account.query.filter_by(id = target_post.posted_on_id, deleted_at = None).first()
    if target_account is None:
        abort(400, "The post you are trying to respond to has been deleted.")
    ...
    new_reply = Reply(request.form.get('respTo'), get_jwt_identity(), request.form.get('textContent'))
...
def delete_reply(reply_id):
    """Deletes the targeted reply if the caller is either the one to create it or the owner of the timeline that the post it is assocaited with."""
    ...
    if (containing_post.posted_on_id == get_jwt_identity() or target.poster_id == get_jwt_identity()):
...
def make_reaction():
    ...
    for reaction in target_post.reactions:
        if reaction.poster_id == get_jwt_identity() and reaction.deleted_at != None:
    ...
    new_reaction = Reaction(res_to=request.form.get('respTo'), poster_id=get_jwt_identity())
...
def delete_reaction(reaction_id):
    ...
    if (containing_post.posted_on_id == requester_id or target.poster_id == requester_id):
        target.deleted_at = datetime.now()
        db.session.commit()
        return "Done", 200
